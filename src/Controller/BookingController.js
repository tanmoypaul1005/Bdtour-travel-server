...
module.exports.addBooking=(req,res,next)=>{
  ...
  const{ name,email,packageID,room_type,address,phone}=req.body; 

  Booking.findOne({email:email ,packageID:packageID})
  .exec((error,data)=>{
   if(data){return next(new ErrorHander("Your All Ready Booking this Package", 500));}

   const _booking=new Booking({name,email,packageID,room_type,address,phone});
